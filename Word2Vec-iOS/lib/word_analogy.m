...
NSDictionary <NSString *, NSNumber *>  * _Nullable  Analogy(NSURL * _Nonnull fileURL,
    ...
    if(numberOfClosest) { N = numberOfClosest.longLongValue; }
    ...
    a = 0;
    while (1) {
        st1[a] = [threeWords cStringUsingEncoding:NSUTF8StringEncoding][a];
        if ((st1[a] == '\n') || (a >= max_size - 1)) {
            st1[a] = 0;
            break;
        }
    ...
    //        if (b == 0) continue;
    //        NSLog(@"\n                                              Word              Distance\n------------------------------------------------------------------------\n");
    NSInteger wordNumber = [threeWords componentsSeparatedByString:@" "].count;
    if (wordNumber == 2) {
    ...
    for (a = 0; a < N; a++) {
        result[[NSString stringWithCString:bestw[a] encoding:NSUTF8StringEncoding]] = @(bestd[a]);
